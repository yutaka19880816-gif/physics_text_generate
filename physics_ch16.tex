...
% 原本と同じく第2章（熱力学）の例題の通し番号を引き継ぐ（第15回が【例題5】〜【例題7】）
\setcounter{nombre}{7}

\subsection{定積モル比熱と定圧モル比熱}
通常の物質に加えるべき熱量は，その物質の比熱を$c$[cal/g・K]，物質を$m$[g]，上昇させる温度を$\Delta T$[K]として$Q=mc\Delta T$[J]で与えられるが，気体を加熱するときに加える熱を考える際，気体の量は[g]ではなく[mol]で，また加える熱量も[cal]ではなく[J]を用いた方が便利である．
{\bf そこで，気体の比熱を，1[mol]の気体を1[K]だけ温度上昇させるのに必要な熱量として定義}する．
これを{\bf モル比熱}という．
% 誤植訂正（2026-09-02）：読点「，」で文が終わっていた（原本 p74 は「という。」）．
モル比熱が$C$[J/mol・K]のとき，$n$[mol]の気体の温度を$\Delta T$[K]だけ上昇させるのに必要な熱量$Q$[J]は
\[Q=nC\Delta T \hspace{2mm} [{\rm J}]\]
で与えられる．
...
の2つの熱の与え方が特に重要で，これらの場合の気体のモル比熱をそれぞれ{\bf 定積モル比熱} \mbox{\boldmath $C_V$} {\bf [J/mol・K]，定圧モル比熱}\mbox{\boldmath $C_P$}{\bf [J/mol・K]}という．

$C_V$と$C_P$は気体の種類によって値が違うが，理想気体の場合は必ず
\[\mbox{\boldmath $C_P-C_V=R$}\]
の関係式を満たす．
これを{\bf マイヤーの関係式}という．

\begin{matome}{定積モル比熱・定圧モル比熱}
 \hspace{3mm} 定積条件で温度を$\Delta T$[K]上昇させるのに必要な熱\hspace{5mm} $Q=nC_V\Delta T $[J]\\
 \hspace{3mm} 定圧条件で温度を$\Delta T$[K]上昇させるのに必要な熱\hspace{5mm} $Q=nC_P\Delta T $[J]\\
...
断面積が$S[{\rm m^2}]$のシリンダーに閉じ込められている気体の圧力を$P$[Pa]とする．
このピストンが微小距離$dl$[m]だけ移動し，シリンダー内の体積が広がるときについて考える．

このとき，気体は外界に対して仕事をしている．
ピストンの移動距離が微小であれば気体が外界に対してなした仕事も微小である．
これを$dW_{out}$[J]とすれば，仕事の定義より
\[dW_{out}=(PS)・dl \hspace{2mm} [{\rm J}] \]
となる．このときの気体の体積は$dV=S・dl [{\rm m^3}]$だけ増加したから，
\[ \mbox{\boldmath $dW_{out}=PdV$}\]
で成立する．
...
\begin{wrapfigure}[5]{r}[5mm]{50mm}
  ...
  {text_p75_fig1.png}
\end{wrapfigure}
この気体がなした仕事を$P-V$図で捉えると，$dW_{out}$[J]は図の微小短冊の面積として表される．
よって，気体が図の曲線上をたどって体積$V:V_1\rightarrow V_2$へと変化するまでに，気体が外界に対してする仕事$W_{out}$[J]は，
\[W_{out}=\int^{V=V_2}_{V=V_1}dW_{out}=\int^{V_2}_{V_1}PdV\]
として求めることができる．

\begin{matome}{気体が外界に対してする仕事}
 \hspace{3mm} $W_{out}=\displaystyle \int^{V_2}_{V_1}PdV$
 \end{matome}
 
 逆に気体が圧縮されるときは，気体は外界から仕事をされている．
 ピストンを十分ゆっくりと動かし，外圧と内圧がほぼつりあいを保ったまま変化させると（これを{\bf 準静的過程}と呼ぶ）外圧は内圧$P$[Pa]と等しく，体積が$-dV[{\rm m^3}]$だけ縮んだとすると気体がなされた仕事$dW_{in}$[J]は
 \[dW_{in}=P・(-dV)\hspace{2mm}[{\rm J}] \]
 で与えられる．
 よって，準静的過程において気体のやりとりした仕事を考える際には気体の内圧を用いて考えればよく，膨張したときには外界に仕事をし，圧縮されたときには外界から仕事をされている．
...
% 誤植訂正（2026-09-21）：終状態の圧力が $P_0+ka/S^2$ になっていた．
%   $P(x)=P_0+kx/S$ に $x=a$ を入れれば $P_0+ka/S$．同じ装置の ⑰[30]（新23の練習［33］）
%   も $P_0+ka/S$ である．図 fig/ch16_ex8_pv.png のラベルも同じ誤植だったので作り直した
%   （texify_draft/mkfig_new23.py）．
...
\subsection{熱力学第一法則と内部エネルギーの変化(1)}
気体の出入りがない（モル数が一定）場合の状態変化を考えよう．
この場合に重要となる物理量は，初めの状態および終わりの状態での{\bf 圧力，体積，温度}と，エネルギーに関しては変化中の{\bf 吸熱量，仕事，内部エネルギー変化}となる．
圧力，体積，温度は状態方程式により，これらのうち2つが分かっていれば残りの1つも決定される．
エネルギーについても，熱力学第一法則より3つの値のうち2つが分かれば残りの1つを求めることができる．

体積一定のまま変化させる{\bf 定積変化}，圧力一定のまま変化させる{\bf 定圧変化}，温度一定のまま変化させる{\bf 等温変化}，熱のやりとりを遮断して変化させる{\bf 断熱変化}の代表的な4つの状態変化について実際に議論してみたい．
解く上で必要な知識をまとめておくと以下のようになる．

\begin{matome}{気体の状態変化}
 \hspace{3mm} \underline{状態方程式}\\
 \hspace{20mm} $PV=nRT$\\
 \hspace{3mm} \underline{熱力学第一法則}\\
 \hspace{20mm} $Q_{in}=\Delta U+W_{out}$ \\
 \hspace{20mm} 気体が外界から{\bf された}仕事を$W(=W_{in})$，気体が外界に{\bf した}仕事を$w(=W_{out})$と書けば\\
 \hspace{25mm} $\Delta U=Q+W \Leftrightarrow \Delta U=Q+(-w)$\hspace{5mm}（$W=-w$）\\
 \hspace{10mm} $\underline{\Delta U[{\rm J}]：内部エネルギー変化}$ \\
 \hspace{25mm} $\Delta U=nC_V\Delta T$ \\
 \hspace{20mm} 状態変化の種類にかかわらずこの表記で表せる． \\ 
 \hspace{20mm} $C_V$[J/mol・K]；定積モル比熱 $\rightarrow$単原子分子理想気体の場合，$C_V=\dfrac{3}{2}R$\\
  \hspace{10mm} $\underline{W_{out}[{\rm J}]：気体が外界にした仕事}$ \\
 \hspace{25mm} $W_{out}=\displaystyle \int^{V_2}_{V_1} PdV$ \\
 \hspace{20mm} $P-V$図における符号付き面積と等しい．\\ 
 \hspace{20mm} 膨張過程において$W_{out}>0$，圧縮過程において$W_{out}<0$\\
  \hspace{10mm} $\underline{Q_{in}[{\rm J}]：吸熱量}$ \\
 \hspace{20mm} 断熱変化：$Q=0$ \\
 \hspace{20mm} 定積変化：$Q=nC_V\Delta T$ \\ 
 \hspace{20mm} 定圧変化：$Q=nC_P\Delta T$\hspace{10mm}（$C_P$[J/mol・K]：定圧モル比熱）
 \end{matome}
...
%%%%%%%%%%%%%%%%%%%%  練習問題（旧 prob_p40.png / prob_p41.png / prob_p42.png）  %%%%%%%%%%%%%%%%%%%%
\setcounter{qno}{16}
...
%%%%%%%%%%%%%%%%%%%%  解答（旧 prob_p126_2.png 〜 prob_p131.png）  %%%%%%%%%%%%%%%%%%%%
...
\ko{1} 次の長方形の面積が求める値である．
       \begin{center}\includegraphics[keepaspectratio,width=46mm]{fig/ch16_a18_1.png}\end{center}
       \[w=p_0(V_1-V_0)\U{J}\Ans\]
\ko{2} 次の台形の面積が求める値である．
       \begin{center}\includegraphics[keepaspectratio,width=46mm]{fig/ch16_a18_2.png}\end{center}
       \[w=\frac{1}{2}(p_0+p_1)(V_1-V_0)\U{J}\Ans\]
\ko{3} 気体の体積変化がないので，
       \[w=0\U{J}\Ans\]
\ko{4} 次の網点部の面積が求める値である．
       \begin{center}\includegraphics[keepaspectratio,width=48mm]{fig/ch16_a18_4.png}\end{center}
       \[\begin{aligned}
...
\ko{5} 気体が外部からされる仕事を$W$とすると，
       \[W=p_0(V_0-V_1)\U{J}\]
       \begin{center}\includegraphics[keepaspectratio,width=46mm]{fig/ch16_a18_5.png}\end{center}
       求めるのは気体が外部にする仕事であるから，
       \[w=-W=-p_0(V_0-V_1)\U{J}\Ans\]
\ko{6} この場合，下の網点部の面積が$W$に等しい．
       \begin{center}\includegraphics[keepaspectratio,width=48mm]{fig/ch16_a18_6.png}\end{center}
       \[\begin{aligned}
...
\ko{3} この気体は圧力を常に大気圧$p_0\U{Pa}$に等しく保って状態変化しているので，この状態変化の$p-V$図は次図の通りとなる．
       \begin{center}\includegraphics[keepaspectratio,width=58mm]{fig/ch16_a19_pv.png}\end{center}
       \hspace*{\fill}\Ans
...
\ko{2} この状態変化は定積変化であるから，$p-V$図上ではグラフは$V$軸に垂直になる．加熱したので$T_1>T_0$であるから，$p_1>p_0$となることに注意すると$p-V$図は次図の通り．
       \begin{center}\includegraphics[keepaspectratio,width=52mm]{fig/ch16_a20_pv.png}\end{center}
       \hspace*{\fill}\Ans
...
\ko{1} 膨張前・膨張後の圧力をそれぞれ$P_1$, $P_2\U{Pa}$として気体の状態方程式を立てると，
       ...
       \begin{center}\includegraphics[keepaspectratio,width=60mm]{fig/ch16_a21_pv.png}\end{center}
       \hspace*{\fill}\Ans
...
\ko{1} 初状態におけるシリンダー内の圧力を$p\U{Pa}$とする．力の作用図は次図の通りであり，ピストンは静止しているので作用する力がつり合っている．
       \begin{center}\includegraphics[keepaspectratio,width=44mm]{fig/ch16_a22_f1.png}\end{center}
       この状態の力のつり合いの式は，
       \[pS=p_0S+Mg\]
       ゆえに，
...
\ko{2} ゆっくりと加熱しているからピストンはゆっくりと動いている．よって，このピストンは力のつり合いを保ちつつ動いていると考えられる．変化途中，ピストンに対して作用する力は，(1)と同じく，大気圧による力，気体の圧力による力，重力の三つである．
       \begin{center}\includegraphics[keepaspectratio,width=44mm]{fig/ch16_a22_f2.png}\end{center}
...
\ko{3} 以上より，この気体の状態変化を$p-V$図で表すと次図の通り．
       \begin{center}\includegraphics[keepaspectratio,width=66mm]{fig/ch16_a22_pv.png}\end{center}
...
\ko{2} 気体の内圧を$P$とすると，このピストンに対しては図のように力が作用している．
       \begin{center}\includegraphics[keepaspectratio,width=66mm]{fig/ch16_a23_f1.png}\end{center}
