# -*- coding: utf-8 -*-
# 新第19回（薄膜・くさび・ニュートンリングの干渉）で必要な図の切り出し。
# 切り出しの実体は mkfig_new16.make（PNGを純Pythonでデコード→窓の中のインクの外接矩形でトリム）。
# 座標は texify_draft/jobs_new19.py に置いてある。
#   使い方:  python3 texify_draft/mkfig_new19.py            … 全部
#            python3 texify_draft/mkfig_new19.py p57 a58    … 名前に含む図だけ
#            python3 texify_draft/mkfig_new19.py --halftone … 網点処理だけやり直す
...
if __name__ == '__main__':
    ...
    from jobs_new19 import jobs, halftone, lighten_from_text
    if not only_ht:
        for j in jobs:
